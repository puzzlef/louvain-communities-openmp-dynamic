...
const RRESLT = /^\{-(.+?)\/\+(.+?) batchf, (.+?) threads\} -> \{(.+?)ms, (.+?)ms preproc, (.+?)ms firstpass, (.+?)ms locmove, (.+?)ms aggr, (.+?) affected, (.+?) iters, (.+?) passes, (.+?) modularity, (.+?) match\} (.+)/m;
...
function readLogLine(ln, data, state) {
  ...
  if (ROMPTH.test(ln)) {
    ...
    var [, batch_deletions_fraction, batch_insertions_fraction, num_threads, time, preprocessing_time, first_pass_time, local_moving_phase_time, aggregation_phase_time, affected_vertices, iterations, passes, modularity, match, technique] = RRESLT.exec(ln);
