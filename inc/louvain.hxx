 ...
 */
template <bool MEASURE=false, class G, class K, class W, class B, class FC, class FA>
inline int louvainMoveW(vector<K>& vcom, vector<W>& ctot, vector<B>& vaff, vector<B>& vaffor, vector<K>& vcs, vector<W>& vcout, const G& x, const vector<W>& vtot, double M, double R, int L, FC fc, FA fa) {
  int l = 0;
  W  el = W();
  for (; l<L;) {
    el = W();
    x.forEachVertexKey([&](auto u) {
      ...
      if (c)      { louvainChangeCommunityW(vcom, ctot, x, u, c, vtot); x.forEachEdgeKey(u, [&](auto v) { vaff[v] = B(1); if (MEASURE) vaffor[v] = B(1); }); }
      vaff[u] = B();
      el += e;  // l1-norm
    });
    if (fc(el, l++)) break;
 ...
 * @returns iterations performed (0 if converged already)
 */
template <bool MEASURE=false, class G, class K, class W, class B, class FC>
inline int louvainMoveW(vector<K>& vcom, vector<W>& ctot, vector<B>& vaff, vector<B>& vaffor, vector<K>& vcs, vector<W>& vcout, const G& x, const vector<W>& vtot, double M, double R, int L, FC fc) {
  auto fa = [](auto u) { return true; };
  return louvainMoveW<MEASURE>(vcom, ctot, vaff, vaffor, vcs, vcout, x, vtot, M, R, L, fc, fa);
}
 ...
 */
template <bool MEASURE=false, class G, class K, class W, class B, class FC, class FA>
inline int louvainMoveOmpW(vector<K>& vcom, vector<W>& ctot, vector<B>& vaff, vector<B>& vaffor, vector<vector<K>*>& vcs, vector<vector<W>*>& vcout, const G& x, const vector<W>& vtot, double M, double R, int L, FC fc, FA fa) {
  size_t S = x.span();
  int l = 0;
  W  el = W();
  for (; l<L;) {
    ...
    for (K u=0; u<S; ++u) {
      ...
      if (c)      { louvainChangeCommunityOmpW(vcom, ctot, x, u, c, vtot); x.forEachEdgeKey(u, [&](auto v) { vaff[v] = B(1); if (MEASURE) vaffor[v] = B(1); }); }
      vaff[u] = B();
      el += e;  // l1-norm
    }
    if (fc(el, l++)) break;
 ...
 */
template <bool MEASURE=false, class G, class K, class W, class B, class FC>
inline int louvainMoveOmpW(vector<K>& vcom, vector<W>& ctot, vector<B>& vaff, vector<B>& vaffor, vector<vector<K>*>& vcs, vector<vector<W>*>& vcout, const G& x, const vector<W>& vtot, double M, double R, int L, FC fc) {
  auto fa = [](auto u) { return true; };
  return louvainMoveOmpW<MEASURE>(vcom, ctot, vaff, vaffor, vcs, vcout, x, vtot, M, R, L, fc, fa);
}
#endif
#pragma endregion
...
inline auto louvainInvoke(const G& x, const LouvainOptions& o, FM fm, FI fi, FA fa) {
  ...
  // Allocate buffers.
  vector<B> vaff(S), vaffor(S);  // Affected vertex flag (first pass)
  ...
  float t  = measureDurationMarked([&](auto mark) {
    ...
    mark([&]() {
      ...
      for (l=0, p=0; M>0 && P>0;) {
        ...
        int m = 0;
        tl += measureDuration([&]() {
          if (isFirst) m = louvainMoveW<true> (vcom, ctot, vaff, vaffor, vcs, vcout, x, vtot, M, R, L, fc, fa);
          else         m = louvainMoveW<false>(vcom, ctot, vaff, vaffor, vcs, vcout, y, vtot, M, R, L, fc);
        });
        l += max(m, 1); ++p;
        if (m<=1 || p>=P) break;
  ...
  return LouvainResult<K, W>(a, utot, dtot, l, p, t, tm/o.repeat, ti/o.repeat, tp/o.repeat, tl/o.repeat, ta/o.repeat, countValue(vaffor, B(1)));
}
...
inline auto louvainInvokeOmp(const G& x, const LouvainOptions& o, FM fm, FI fi, FA fa) {
  ...
  float t  = measureDurationMarked([&](auto mark) {
    ...
    mark([&]() {
      ...
      for (l=0, p=0; M>0 && P>0;) {
        ...
        int m = 0;
        tl += measureDuration([&]() {
          if (isFirst) m = louvainMoveOmpW<true> (vcom, ctot, vaff, vaffor, vcs, vcout, x, vtot, M, R, L, fc, fa);
          else         m = louvainMoveOmpW<false>(vcom, ctot, vaff, vaffor, vcs, vcout, y, vtot, M, R, L, fc);
        });
        l += max(m, 1); ++p;
        if (m<=1 || p>=P) break;
        size_t GN = isFirst? x.order() : y.order();
  ...
  return LouvainResult<K, W>(a, utot, dtot, l, p, t, tm/o.repeat, ti/o.repeat, tp/o.repeat, tl/o.repeat, ta/o.repeat, countValueOmp(vaffor, B(1)));
}
#endif
#pragma endregion
